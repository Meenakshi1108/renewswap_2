App = {
  ...
  contracts: {},
  names: new Array(),
  address: '0xec629661fb85e8c8844c37bdf2d5b0b5ddef259b',
  url: 'https://sepolia.infura.io/v3/3fc60b1b45c1433f8ee141fc8f751a7a',
  network_id: 11155111,
  chairPerson: "0xA4dbDFc4A478501E0247BBD65483A8E95387a348",
  currentAccounts: null,
  currentAccount: null,
  currentAccountRole: null,
  addressListFromContract:null,
  ...
  initContract: async function () {
	...
	$.getJSON('RenewableEnergyToken.json', function(data) {      
	  ...
	  jQuery('#current_account').text(App.currentAccount[0]);
		...
		App.contracts.vote.methods.getAddressList().call().then((result)=>{
		
			App.addressListFromContract=result;
			var option='<option></option>';
		
			for(var i=0;i<App.addressListFromContract.length;i++){
				option+='<option>'+App.addressListFromContract[i]+'</option>'; 
			}
			console.log("OPTIONS");
			console.log(option);
  ...
  handleBuyREC: async function () {
	...
	App.contracts.vote.methods.recs(assetId)
      ...
      .then((r)=>{
        ...
        .send({from:App.currentAccount[0],value: r.price})
        .on('receipt',(receipt)=>{
          console.log(receipt)
        })
        .on('transactionHash',(hash)=>{
